...
from torchattacks.attacks.pgd_uap import *
...
sys.stdout = Logger('output_v2_mprompt_25train_100test_PGD_64_200_A.log', sys.stdout)
...
save_img_path = "MiniGPTv2_Single_Img_multi_goal_adv_img/train_size_" + str(
...
for p in range(model.train_num):
    print('-------------------------Response Goal ' + str(model.train_goal_index[p] + 1) + ':' + model.mprompt[
        p] + '----------------------------------------')
...
with open('rst_v2_mprompt_' + str(model.train_num) + '_Test_goal_output_' +attack_mode+'_'+str(attack_power)+'_'+str(attack_iters)+'_'+round+ '.csv', 'w', encoding='utf-8',
          newline='') as f:
    write = csv.writer(f)
    rr = 0
