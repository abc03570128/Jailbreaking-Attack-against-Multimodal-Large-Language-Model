...
from torchattacks.attacks.pgd_uap_noise_v1 import *
...
sys.stdout = Logger('output_v1_Mprompt_Mimage_10train_Img_10test_Img_25train_prompt_5test_prompt_PGD_64_500_class_7.log', sys.stdout)
...
tensor_image = torch.zeros((image_batch_size, 3, 224, 224))
...
save_noise_path = "v1_Mprompt_Mimage_universal_noise_img/class_"+str(class_flag) +'_'+ str(prompt_train_num) + '_train_prompt_' + str(train_img_num) + '_train_image' + attack_mode + "_" + str(attack_power) + "_" + str(attack_iters) + ".jpg"
adv_noise = denorm(universal_noise)
torch.save(adv_noise, 'adv_noise_pt_data/'+str(class_flag)+'.pt')
...
for j in range(train_img_num):
    save_img_path = "MiniGPTv1_Multi_Prompt_Multi_Img_adv_img/" + str(class_flag) + '/' +' Train_img_' + str(train_rnd_idx[j]) + ".jpg"
...
with open('rst_v1_Mprompt_Mimage_class_' + str(class_flag)+'_Train_goal_output.csv', 'w', encoding='utf-8', newline='') as f:
    write = csv.writer(f)
    rr = 0
...
for j in range(test_img_num):
    save_img_path = "MiniGPTv1_Multi_Prompt_Multi_Img_adv_img/" + str(class_flag) + '/' + 'Test_img_' + str(test_idx[j]) + ".jpg"
...
with open('rst_v1_Mprompt_Mimage_class_' + str(class_flag) + '_Test_goal_output.csv', 'w', encoding='utf-8', newline='') as f:
    write = csv.writer(f)
    rr = 0
    write.writerow(["=======================================================Class" + str(class_flag) + "======================================================="])
